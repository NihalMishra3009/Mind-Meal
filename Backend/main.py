...
app.add_middleware(
    ...
    allow_origins=[
        ...
        "https://946a79dc.mind-meal.pages.dev",
    ],
...
async def generate_flux_image(prompt: str) -> dict:
    ...
    payload = {"prompt": prompt, "image_size": "square"}
...
async def suggest_meal(ingredients: str):
    ...
    for recipe in RECIPES:
        recipe_ing_raw = recipe["ingredients"]
        matched = [orig for orig in recipe_ing_raw if normalize(orig) in user_ingredients]
        missing = [orig for orig in recipe_ing_raw if normalize(orig) not in user_ingredients]
